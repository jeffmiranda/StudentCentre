...
class scClassLevelGetList extends modObjectGetListProcessor {
    ...
    public function prepareQueryBeforeCount(xPDOQuery $c) {
		...
		if (!empty($categoryId)) {
		    ...
		    // if scheduledClassId parameter exists,
			$scheduledClassId = $this->getProperty('scheduled_class_id');
			if (!empty($scheduledClassId)) {
				$scheduledClass = $this->modx->getObject('scScheduledClass', $scheduledClassId);
				$class = $scheduledClass->getOne('Class');
				$c->where(array(
		            'scClassLevel.class_level_category_id' => $class->get('class_level_category_id')
		        ));
		    }
	    }
