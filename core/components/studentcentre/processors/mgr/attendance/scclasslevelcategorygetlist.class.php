<?php
class scClassLevelCategoryGetList extends modObjectGetListProcessor {
    ...
    public function prepareQueryBeforeCount(xPDOQuery $c) {
	    ...
	    if (!empty($query)) {
	        $c->where(array(
	            'scClassLevelCategory.name:LIKE' => '%'.$query.'%'
	            ,'OR:scClassLevelCategory.description:LIKE' => '%'.$query.'%'
	        ));
	    }
		...
		if (!empty($activeOnly)) {
	        $c->where(array(
	            'scClassLevelCategory.active' => $activeOnly
	        ));
	    }
...
return 'scClassLevelCategoryGetList';